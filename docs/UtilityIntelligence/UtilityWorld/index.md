...
A Utility World contains a collection of [[utility-entities|Utility Entities]] and the ID of each Utility Entity is only unique within its own world. Therefore, utility entities in different worlds might have the same IDs. 

The main jobs of a Utility World are:
1. Handling the decision-making process of all [[utility-agents|Utility Agents]] inside the world.
2. Running the task associated with the chosen decision for all [[utility-agents|Utility Agents]]  inside the world.
